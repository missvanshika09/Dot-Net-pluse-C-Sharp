...
class Temp
{
    public Temp() // constructor for inner loop 
    {
...
class JoinDemo
{
    static void Main()
    {
        ...
        // inner join 
        IEnumerable<Temp> inStockList = from item in items
                                        join entry in statusList
                          on item.ItemNumber equals entry.ItemNumber
                                        select new Temp { Name = item.Name, InStock = entry.InStock };
        /*another way to write inner loop 
        var q = items.Join(statusList,
            it => it.ItemNumber,
             sl => sl.ItemNumber,
(it, sl) => new Temp(it.Name, sl.InStock), new TestComparer());
        done with ineer loop */
